<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DepartmentProductListing.ascx.cs" Inherits="UC.UI.Controls.DepartmentProductListing" %>
...
<%--                |
    ...
    <asp:DataList ID="dlstProducts" EnableTheming="False" runat="server" DataKeyField="ProductID"
        Width="100%" RepeatColumns="2" RepeatDirection="Horizontal" CellPadding="0" CellSpacing="0"
        ...
        <ItemTemplate>
            <mb:ProductView runat="server" ID="ProductView" Title='<%# Eval("Title") %>' ProductID='<%# Eval("ProductID") %>'
                ...
                DiscountPercentage='<%#Eval("DiscountPercentage")%>' CurrencyID='<%#Eval("CurrencyID")%>' UnitPrice='<%#Eval("UnitPrice")%>'

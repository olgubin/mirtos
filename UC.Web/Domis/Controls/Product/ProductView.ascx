...
<table style="width: 100%">
    <tr>
        <td style="width:187px">
            <asp:HyperLink runat="server" ID="HyperLink2" NavigateUrl='<%# SeoHelper.GetAbsoluteUrl(this.ResolveUrl("~/ShowProduct.aspx?ID=" + ProductID)) %>'
                ToolTip='<%# Title %>'>
                <%--<asp:Image runat="server" ID="Image1" BorderWidth="1px" GenerateEmptyAlternateText="true" ImageUrl='<%# ImageURL %>' Width="100px" Height="100px" BorderColor="#9aaab1" />--%>
                <asp:Image runat="server" ID="Image2" BorderWidth="1px" GenerateEmptyAlternateText="true"
                    ImageUrl='<%# SeoHelper.GetAbsoluteUrl(this.ResolveUrl("~/smallimage.aspx?ID=" + ProductID)) %>'
                    Width="177px" Height="267px" BorderColor="#fef7f8" />
            </asp:HyperLink>
        </td>
        <td class="producttitle">
            ...
            <asp:HyperLink runat="server" ID="lnkProductTitle" Text='<%# Title %>' NavigateUrl='<%# SeoHelper.GetAbsoluteUrl(this.ResolveUrl("~/ShowProduct.aspx?ID=" + ProductID)) %>' />
            <div>
            ...
            <%--            <asp:Panel runat="server" Visible='<%# ManufacturerID==0 ? false : true %>'>
                ...
                <asp:HyperLink runat="server" ID="lnkManTitle" Text='<%# ManufacturerTitle %>' NavigateUrl='<%# "~/Departments.aspx?ManID=" + ManufacturerID %>' Font-Bold="false" />
            </asp:Panel>--%>
            <div class="prodattr">
                <mb:ProductAttributes runat="server" ID="tblProductAttributes" ProductID='<%# ProductID %>' />
            </div>
            <div class="s_prodprice">
                <asp:Panel runat="server" ID="pnlToCart" Visible='<%# UnitPrice > 0 & UnitsInStock > 0 %>'>
                    <table>
                        <tr>
                            <th>
                                <%# (this.Page as BasePage).FormatPrice(FinalPrice) %>
                                <asp:Panel ID="Panel1" runat="server" Visible='<%# DiscountPercentage > 0 %>'>
                                    <s>
